...
Preprocessor::Preprocessor(const CDNF_formula &clauses, const CDNF_formula &terms) : qhead(0), maxvar(0),
                                                                                     empty_seen(false),
                                                                                     nr_constraints{0, 0}, nr_units(0),
                                                                                     nr_pure(0), nr_blocked(0),
                                                                                     nr_literals_reduced(0),
                                                                                     nr_self_subsumed(0),
                                                                                     nr_subsumed(0), nr_eliminated(0) {
    // Store each clause and term as a set of literals.
    for (const auto &clause: clauses) {
        addConstraint(clause, false);
        auto max_var_it = std::max_element(clause.begin(), clause.end(), [](int l1, int l2) {
            return abs(l1) < abs(l2) || (abs(l1) == abs(l2) && l1 < l2);
        });
    ...
    for (const auto &term: terms) {
        ...
        auto max_var_it = std::max_element(term.begin(), term.end(), [](int l1, int l2) {
            return abs(l1) < abs(l2) || (abs(l1) == abs(l2) && l1 < l2);
        });
        maxvar = std::max(maxvar, abs(*max_var_it));
        touchAll(term, true);
...
template<class T>
bool Preprocessor::resolventTautological(const T &c, int pivot_variable) const {
...
template<class T>
bool Preprocessor::isBlocked(const T &c, bool ctype) {
    for (const auto &l: c) {
...
std::vector<int> Preprocessor::findSubsumed(const std::vector<int> &c, bool ctype) const {
    ...
    int l_occurs_fewest = *std::min_element(c.begin(), c.end(),
                                            [this, ctype](int l1, int l2) {
                                                return lit_to_occurrences[ctype].at(l1).size() <
                                                       lit_to_occurrences[ctype].at(l2).size();
                                            });
    for (auto other_index: lit_to_occurrences[ctype].at(l_occurs_fewest)) {
        auto &c_other = index_to_lits[ctype].at(other_index);
        if (&c_other != &c && c.size() <= c_other.size() &&
            std::includes(c.begin(), c.end(), c_other.begin(), c_other.end())) {
...
Preprocessor::incidentToLiterals(const std::set<std::pair<int, bool>> &literal_type_pairs) {
...
std::set<std::pair<int, bool>>
Preprocessor::incidentToVariables(const std::set<std::pair<int, bool>> &variable_type_pairs) {
