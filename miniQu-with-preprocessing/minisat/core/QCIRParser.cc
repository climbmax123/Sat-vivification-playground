...
QCIRParser::QCIRParser(const string &filename) {
    ...
    while (std::getline(file, line)) {
        ...
        line.erase(remove_if(line.begin(), line.end(), [](char c) { return isspace(int(c)); }),
                   line.end()); // Remove whitespaces.
...
void QCIRParser::readQuantifierBlock(const string &line) {
    ...
    VariableType type = (quantifier_string == QBFParser::EXISTS_STRING) ? VariableType::Existential
...
void
QCIRParser::mapFormula(std::vector<std::vector<int>> &formula, const std::unordered_map<int, Minisat::Var> &var_map,
                       bool ctype) {
    for (auto &c: formula) {
        for (auto &l: c) {
            auto v = abs(l);
            bool is_universal = (v < variable_gate_boundary && gates[v].gate_type == GateType::Universal) ||
                                (variable_gate_boundary <= v && ctype);
