...
/**
 * @brief This method performs the preprocessing algorithm to simplify the CDNF formula
*/
...
/**
 * @brief This method returns a pair of CDNF_formula objects representing the simplified clauses and terms in CNF
 * @return std::pair<CDNF_formula, CDNF_formula> A pair of CDNF_formula objects representing the simplified clauses and terms in CNF
*/
std::pair<CDNF_formula, CDNF_formula> Preprocessor::getClausesTerms() {
  CDNF_formula clauses, terms;
  clauses.reserve(index_to_lits[false].size());
  terms.reserve(index_to_lits[true].size());

  // Iterate through the index_to_lits map for clauses and add them to the clauses vector
  for (auto& [_, clause]: index_to_lits[false]) {
    clauses.emplace_back(std::make_move_iterator(clause.begin()), std::make_move_iterator(clause.end()));

    assert(empty_seen || clauses.back().size() != 1);
  }
  // Iterate through the index_to_lits map for terms and add them to the terms vector
  for (auto&& [_, term]: index_to_lits[true]) {
    terms.emplace_back(term.begin(), term.end()); // Note: Maby use std::move?
    assert(empty_seen || terms.back().size() != 1); // Ensure that the term is not empty or a unit term
  }
  return std::make_pair(clauses, terms);
}

/**
 * @brief This method adds a constraint to the index_to_lits and lit_to_occurrences maps
 *
 * @param c A vector representing the constraint to be added
 * @param ctype A boolean indicating whether the constraint is a term (true) or a clause (false)
 */
void Preprocessor::addConstraint(const std::vector<int>& c, bool ctype) {
  ...
  added.insert(std::make_pair(index, ctype)); // NOTE: MABY use emblace back
  for (auto l: c_copy) {
...
/**
 * @brief This method removes a constraint from the index_to_lits and lit_to_occurrences maps
 *
 * @param index The index of the constraint to be removed
 * @param ctype A boolean indicating whether the constraint is a term (true) or a clause (false)
 * @param check_pure A boolean indicating whether to check for pure literals after removing the constraint
 */
void Preprocessor::removeConstraint(int index, bool ctype, bool check_pure) {
  assert(index_to_lits[ctype].count(index) == 1);
...
/**
 * @brief This method removes a literal from a constraint
 *
 * @param index The index of the constraint
 * @param l The literal to be removed
 * @param ctype A boolean indicating whether the constraint is a term (true) or a clause (false)
 * @return true if the constraint becomes empty after removing the literal, false otherwise
 */
...
/**
 * @brief Propagates unit literals and disabled clauses/terms until no more propagation is possible
 */
void Preprocessor::propagate() {
  ...
  while (qhead < trail.size()) {
    ...
    // Propagate the unit literal by removing it from each clause/term in which it occurs
    for (int i = 0; i < 2; i++) {
      bool ctype = i == 0 ? false : true;
      if (lit_to_occurrences[ctype][l].empty()) {
        continue;
      }

      // Iterate over the occurrences of the unit literal
      auto it = lit_to_occurrences[ctype][l].begin();
      while (it != lit_to_occurrences[ctype][l].end()) {
        int index = *it;

        // Strengthen clause/term by removing the unit literal
        if (removeLit(index, l, ctype)) {
          // If the clause/term becomes empty, an empty clause/term is detected and an exception is thrown
          empty_seen = true;
          throw ctype;
        }

        // Remove the occurrence of the unit literal
        it = lit_to_occurrences[ctype][l].erase(it);
      }
    }

    // Remove any disabled clauses/terms in which the negation of the unit literal occurs
    for (int i = 0; i < 2; i++) {
      bool ctype = i == 0 ? false : true;
      if (lit_to_occurrences[ctype][neg_l].empty()) {
        continue;
      }

      // Iterate over the occurrences of the negation of the unit literal
      auto it = lit_to_occurrences[ctype][neg_l].begin();
      while (it != lit_to_occurrences[ctype][neg_l].end()) {
        int index = *it;

        // Delete the negation of the unit literal from the clause/term to avoid a redundant purity check
        auto& c = index_to_lits[ctype][index];
        auto neg_l_it = std::find(c.begin(), c.end(), neg_l);
        assert(neg_l_it != c.end());
        c.erase(neg_l_it);

        // Remove the disabled clause/term
        removeConstraint(index, ctype, true);

        // Remove the occurrence of the negation of the unit literal
        it = lit_to_occurrences[ctype][neg_l].erase(it);
      }
    }
  }
}
