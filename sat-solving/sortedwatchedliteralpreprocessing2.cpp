...
namespace sorted2 {
    ...
    void
    vivify_with_sorted_pure_lit(CDNF_formula &cnf, int timeLimitInSeconds, std::string path, std::string filename) {

        // We want to track the runtime to stop running after timeout.
        auto startTime = std::chrono::steady_clock::now();


        std::cout << "Started Preparing Formula for Vivify: Here " << std::endl;
        ...
        std::cout << "\t UP. \t\t\t\t Time Consumed: " << std::to_string(time_needed) << " ms"
                  << "\t\t Reduced Clauses: "
        ...
        std::cout << "\t Pure E.\t\t\t Time Consumed: " << std::to_string(time_needed) << " ms"
                  << "\t\t Reduced Clauses: "
                  << cnf_clauses - cnf.size() << std::endl;
        ...
        while (change) {
            ...
            while (step < cnf.size()) {
                if (step % static_cast<int>(cnf.size() / 4) == 0) {
                    ...
                    time_needed = duration_cast<milliseconds>(high_resolution_clock::now() - start_sort_time).count();
                    std::cout << "\t Sorting: \t\t\t Time Consumed: " << std::to_string(time_needed) << " ms"
                              << std::endl;
                ...
                if (step % 100 == 0) {
                    ...
                    std::cout << "-> " << step << ":" << "rm clauses: " << removed_clauses << ",\t rm lits: "
                              << removed_literals
                              << ",\t found pures: " << found_pures << ",\t time elapsed: " << elapsedSeconds << "s"
                              << std::endl;
                }
                ...
                while (!shortened && c != cb && cb.size() < c.size()) {
                    ...
                    if (is_unsat) {
                        ...
                        if (c != cb) {
                            ...
                            if (!pures.empty()) {
                                pureRuntimeLiteralElimination(cnf, mapping, std::move(pures),
                                                                      found_pures,
