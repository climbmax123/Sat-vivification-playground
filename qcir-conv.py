...
def main():
    ...
    def TransmogrifyProb(p):
            ...
            while again:
                ...
                for (old, new) in xlat.items():
                    if old != new:
                        ...
                        del p.RepTable[old]
                for (RepVar, expr) in p.RepTable.items():
                    p.RepTable[RepVar] = tuple([(xlat.get(x,x) if int(x) > 0 else -xlat.get(-x,-x))
                                                for x in expr])
                del xlat
        ...
        if glo.ConvOr and len(p.QuantPfx.keys())==1 and (not OnlyAnd):
            FindParents(p)
            for RepVar, Expr in sorted(p.RepTable.items(), key=lambda k_v: -p.LevByVar[k_v[0]]):
                for arg in Expr[1:]:
