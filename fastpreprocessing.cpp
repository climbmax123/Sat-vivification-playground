...
void FastUnitPropagation(CDNF_formula& cnf) {
    bool progress = true;
    while (progress) {
        progress = false;
...
int fast_select_a_literal(const std::vector<int> &c, const std::vector<int> &cb) {
    for (int lit: c) {
    ...
    while (change) {
        ...
        for (int i = 0; i < cnf.size(); i++){
            ...
            while (!shortened && c != cb) {
                int l = fast_select_a_literal(c, cb);
                cb.push_back(l);
                cnf_backup.push_back({-l});
                std::vector<std::pair<int,int>> found_unit_clauses;
                FastUP(cnf_backup, found_unit_clauses);
                if (cnf_backup.empty()) {
                    cnf = CDNF_formula();
...
void FastVivifyWithPureLit(CDNF_formula& cnf){
    ...
    while (change) {
        change = false;
        FastUnitPropagation(cnf);
        FastPureLiteralElimination(cnf);
        for (int i = 0; i < cnf.size(); i++){
            std::vector<int> c = cnf[i];
            cnf.erase(cnf.begin() + i);
            CDNF_formula cnf_backup = cnf;
            ...
            while (!shortened && c != cb) {
                ...
                FastPureLiteralElimination(cnf_backup);
                if (cnf_backup.empty()) {
                    cnf = CDNF_formula();
                    return;
                }
